import { make_get_request, make_post_request } from './requests.js';
import { correctMinutesSeconds } from './extras.js';
export let tokenRefreshed = false;
...
export async function get_user_information() {
	...
	let response = await make_get_request(
		'https://api-auth.emel.pt/user',
		user.accessToken,
	);
	user = { ...user, ...response.data };

	// Update user image based on user details
	if (document.getElementById('userPicture')) document.getElementById(
		'userPicture',
	).innerHTML = `<img src="https://ui-avatars.com/api/?name=${encodeURI(
		user.name,
	)}&size=${
		document.documentElement.clientHeight * 0.14
	}&background=ffffff&color=79C000&rounded=true">`;
...
export async function openUserSettings() {
	...
	const userObj = await get_user_information();
	...
	const expirationDateFormatted = subscriptionExpiration.getDate() + '/' + (subscriptionExpiration.getMonth() + 1) + '/' + subscriptionExpiration.getFullYear();

	// Calculate the total time of the tripHistory
	let totalTime = 0;
	for (const trip of userObj.tripHistory) {
	...
	totalTime.setTime(
		totalTime.getTime() + totalTime.getTimezoneOffset() * 60 * 1000,
	...
	let co2Saved = Math.floor(hoursFloat * 15 * 0.25); // hours * km in 1 hour * kg of co2 saved per km
	...
	settingsElement.innerHTML = `
        ...
        <div id="balanceAndBonusContainer">
            <div id="balanceContainer">
                ...
                <div id="balance">${parseFloat(userObj.balance).toFixed(
		2,
	)}€</div>
            </div>
