import { loadStationMarkersFromArray } from './map.js';
import { make_post_request, checkToken, startWSConnection } from './requests.js';
import { user, get_user_information } from './user.js';
let stationsArray;
let lastStationObj;
...
export async function openStationMenu(stationSerialNumber) {
	...
	// get station object
	let stationObj = stationsArray.filter(obj => {
	...
	if (navigation.active) menu.style.zIndex = 99;
	...
	if (typeof bikeList != 'undefined' && typeof dockList != 'undefined') {
		menu.innerHTML = `
            ...
            <div id="availableBikesNumber">${
	numBikes === 1 ?
		'1 bicicleta disponível' :
		`${numBikes} bicicletas disponíveis`
}</div>
...
async function openBikeList(stationSerialNumber) {
	...
	for (let bike of stationObj.bikeList) {
		...
		bikeListElement.innerHTML = `
            ${
	bike.name[0] == 'E' ?
		`${bike.name} - ${bike.battery}% - Doca ${dockObj.name}` :
	...
	if (document.getElementById('bikeList').childElementCount == 0) document.getElementById('bikeList').innerHTML =
      'Não há bicicletas na estação.';
