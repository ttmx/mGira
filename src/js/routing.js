import { get_stations } from './stations';
...
async function searchPlace() {
	...
	if (query.length > 2) {
		...
		try {
			...
			if (document.getElementById('placeList') && document.getElementById('placeList').childElementCount == 0) document.getElementById('placeList').innerHTML = 'Nenhum lugar encontrado.';
		} catch (err) {
			console.log(err);
			/*console.log("An error occurred: " + err.status)
            console.error(await err.response.json())*/
...
export function viewRoute(toCoordinates) {
