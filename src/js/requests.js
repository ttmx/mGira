import { user, token_refresh, tokenRefreshed } from './user.js';
import { delay } from './extras.js';
import { ratedTripsList, trip, openRateTripMenu, cancel_bike_reserve, tripTimer } from './bikes.js';
import { navigation } from './navigation.js';
let ws;

export async function make_post_request(url, body, accessToken = null) {
	let responseObject;
	...
	if (response.status == 401) {
      ...
      responseObject.error.message == 'Invalid credentials.'
		) {
		...
		if (responseObject.errors[0].message == 'trip_interval_limit') {
			...
			return;
		} else if (responseObject.errors[0].message != 'Error executing document.') alert(responseObject.errors[0].message);
...
export async function make_get_request(url, accessToken = null) {
	let response = await fetch(url, {
		method: 'GET',
		headers: {
			'X-Authorization': `Bearer ${accessToken}`,
...
export function startWSConnection(force = false) {
	...
	ws.onmessage = function (msg) {
		...
		if (typeof msg.data != 'undefined') {
          ...
          Object.hasOwn(msgObj.payload.data, 'activeTripSubscription')
					...
					if (
						activeTripObj.code != 'no_trip' &&
            activeTripObj.code != 'unauthorized'
					) {
						// Real trip info
						if (
							activeTripObj.finished === true &&
              !ratedTripsList.includes(activeTripObj.code)
						) {
							// End trip
							trip.ended = true;
							cancel_bike_reserve();
							...
							if (
								!document.querySelector('#tripOverlay') &&
                !navigation.active
							) {
							...
							else if (navigation.active && !trip.timerRunning) {
								// start the trip timer
								trip.ended = false;
								tripTimer(new Date(activeTripObj.startDate));
							}
					...
					Object.hasOwn(msgObj.payload, 'errors') &&
          msgObj.payload.errors
				) {
...
export function checkToken(callback) {
	if (tokenRefreshed === false) setTimeout(
		checkToken.bind(
			null,
			callback,
			arguments[1],
