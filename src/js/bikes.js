import { createCustomTextPrompt, createCustomYesNoPrompt } from './dialogs';
import { appendElementToBodyFromHTML } from './extras';
import { make_post_request } from './requests';
export const trip = {
	ended: true,
	timerRunning: false,
};
export let ratedTripsList = [];
...
async function reserve_bike(serialNumber) {
	response = await make_post_request(
		...
		JSON.stringify({
			...
			query:
        'mutation reserveBike($input: String) {reserveBike(input: $input)}',
		}),
		user.accessToken,
	);
...
export async function cancel_bike_reserve() {
	response = await make_post_request(
		'https://apigira.emel.pt/graphql',
		JSON.stringify({
...
async function get_active_trip() {
	response = await make_post_request(
		...
		JSON.stringify({
			...
			variables: {},
			query:
        'query activeTrip {activeTrip {code, startDate, endDate, cost, client, tripStatus, version}}',
		}),
		user.accessToken,
	);
...
async function trip_pay_with_no_points(tripCode) {
	response = await make_post_request(
		...
		JSON.stringify({
			...
			query:
        'mutation tripPayWithNoPoints($input: String) { tripPayWithNoPoints(input: $input) }',
		}),
		user.accessToken,
	);
...
async function trip_pay_with_points(tripCode) {
	response = await make_post_request(
		...
		JSON.stringify({
			...
			variables: { input: tripCode },
			query:
        'mutation tripPayWithPoints($input: String) { tripPayWithPoints(input: $input) }',
		}),
		user.accessToken,
	);
...
function openUnlockBikeCard(
	...
	if (stationSerialNumber != null) {
		...
		if (
			!(
				distance(pos[1], pos[0], stationObj.latitude, stationObj.longitude) < 25
			)
		) {
	...
	card.innerHTML = `
        <div id="bikeReserveCard">
            ...
            <div id="textContent">
                ${
	bikeObj.name[0] == 'E' ?
		`${bikeObj.name}<br>${bikeObj.battery}% de bateria` :
		`${bikeObj.name}`
}
                <br><br>
                ${lastStationObj.name}
	...
	if (navigation.active) card.style.zIndex = 99;
}
...
function openTakeUnregisteredBikeMenu(stationSerialNumber) {
	...
	if (navigation.active) document.getElementById('takeUnregisteredBike').style.zIndex = 99;
}
...
function takeUnregisteredBike() {
	...
	let bikeObj = bikeSerialNumberMapping.filter(
		bike => bike.name == bikeName,
	)[0];
...
export function tripTimer(startTime) {
	if (tripEnded === false) {
		...
		if (document.querySelector('#tripTime')) {
			...
			elapsedTime.setTime(
				elapsedTime.getTime() + elapsedTime.getTimezoneOffset() * 60 * 1000,
			); // Correct because of Daylight Saving Time
...
export function openRateTripMenu(tripObj) {
	// Calculate the trip time
	let elapsedTime = new Date(
		new Date(tripObj.endDate) - new Date(tripObj.startDate),
	);
	elapsedTime.setTime(
		elapsedTime.getTime() + elapsedTime.getTimezoneOffset() * 60 * 1000,
	); // Correct because of Daylight Saving Time
	...
	tripObj.cost,
                ...
                <br><br>
                +${tripObj.tripPoints} <i class="bi bi-arrow-right"></i> ${
	tripObj.clientPoints
} pontos totais
            </div>
            ...
            <div id="sendButton" onclick="rateTrip('${tripObj.code}',${
	tripObj.cost
})">Enviar</div>
        </div>
    </div>
	...
	if (tripCost != 0) {
		createCustomYesNoPrompt(
			...
			async () => {
				...
				if (await trip_pay_with_points(tripCode) !== 0) alert('Não foi possível pagar a viagem.');
			},
			async () => {
				// the success response is a 0
		...
		if (!await trip_pay_with_no_points(tripCode)) alert('Não foi possível pagar a viagem.');
	}
}